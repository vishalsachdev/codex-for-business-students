...
- Next: Throwaway repo `novabrew-quiz-deploytest` still needs deletion (requires interactive `gh auth refresh -s delete_repo`). Optional pre-class item deferred: decide manual-vs-GitHub-Actions deploy for student projects, then build the workflow.
